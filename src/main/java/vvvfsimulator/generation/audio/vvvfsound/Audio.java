...
public final class Audio {
    ...
    public static void exportWavLine(GenerationParameter param, int samplingFreq, String path) throws IOException {
        exportWavFile(param, samplingFreq, path, Mode.Line);
    }

    public static void exportWavPhaseCurrent(GenerationParameter param, int samplingFreq, String path) throws IOException {
        exportWavFile(param, samplingFreq, path, Mode.PhaseCurrent);
    }

    private static void exportWavFile(GenerationParameter parameter, int samplingFreq, String path, Mode mode) throws IOException {
        double dt = 1.0 / samplingFreq;
        parameter.progress.total = parameter.baseFrequencyData.getEstimatedSteps(dt);
        Struct.Domain domain = new Struct.Domain(parameter.trainData.motorSpec);
        ...
        try (BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(path))) {
            ...
            while (true) {
                ...
                Struct.PhaseState value = Common.calculatePhsaseState(domain, mode == Mode.Line ? MyMath.M_PI_6 : 0);
                double sample = switch (mode) {
                    case Line -> (value.u - value.v) / 2.0;
                    case PhaseCurrent -> (2.0 * value.u - value.v - value.w) / 4.0;
                };
                short pcm = (short) Math.max(Math.min(sample * 0.35 * Short.MAX_VALUE, Short.MAX_VALUE), Short.MIN_VALUE);
                out.write(pcm & 0xFF);
