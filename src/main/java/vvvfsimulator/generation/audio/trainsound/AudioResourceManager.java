...
public final class AudioResourceManager {
    public static final String SAMPLE_IR_PATH = "/assets/createvvvfsim/TrainSound/Filter.wav";

    private AudioResourceManager() {
    }

    public static float[] readResourceAudioFileSample(String path, int[] sampleRateOut) {
    ...
    public static float[] resampleLinear(float[] input, int inputSampleRate, int outputSampleRate) {
        if (input == null || input.length == 0 || inputSampleRate <= 0 || outputSampleRate <= 0 || inputSampleRate == outputSampleRate) {
    ...
    private static float[] decodePcm16WavMono(byte[] bytes, int[] sampleRateOut) {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            out[i] = (sum / (float) channels) / Short.MAX_VALUE;
        }
