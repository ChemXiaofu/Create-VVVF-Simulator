...
public final class Analyze {
    ...
    private static int getPointAtNum(double time, StructCompiled compiled) {
        ...
        while (true) {
            StructCompiled.Point point = compiled.points.get(pos);
    ...
    private static StructCompiled.Point getPointAtData(double time, StructCompiled compiled) {
    ...
    private static double getFreqAt(double time, double initial, StructCompiled compiled) {
        StructCompiled.Point p = getPointAtData(time, compiled);
    ...
    public static boolean checkForFreqChange(vvvfsimulator.vvvf.model.Struct.Domain control,StructCompiled data,Struct soundData,double deltaTime) {
        ...
        StructCompiled.Point target = data.points.get(index);
        StructCompiled.Point next = index + 1 < data.points.size() ? data.points.get(index + 1) : null;
        StructCompiled.Point prev = index - 1 >= 0 ? data.points.get(index - 1) : null;

        boolean braking = !target.isAccel;
        boolean isPowerOn = target.isPowerOn;
        if (!isPowerOn && prev != null) {
            braking = !prev.isAccel;
        }

        if (next != null && control.isFreeRun() && next.isPowerOn) {
            double powerOnFrequency = getFreqAt(target.endTime, 0, data);
            double freqPerSec = !next.isAccel ? soundData.jerkSetting.braking.on.frequencyChangeRate : soundData.jerkSetting.accelerating.on.frequencyChangeRate;
            double freqGoto = !next.isAccel ? soundData.jerkSetting.braking.on.maxControlFrequency : soundData.jerkSetting.accelerating.on.maxControlFrequency;
        ...
        if (forceOnFrequency != -1) {
            ...
            if (control.isBaseWaveTimeChangeAllowed()) {
                control.multiplyBaseWaveTime(forceAmp);
            }
        }
