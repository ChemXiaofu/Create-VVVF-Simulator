...
8. 构建GUI，在游戏内配置vvvf策略
9. 移植解析外部yaml配置模块，实现与vvvf-sim配置互通
10. 继续移植原vvvf-sim更多可配置属性
11. (Opt.)音频重载指令（已完成）
12. (Opt.)与sound physics相关模组联动
13. (Opt.)在列车站《组装列车》界面为每辆车单独配置策略
14. (Opt.)列车控制改为功率控制
15. (Opt.)修改其他生物的驾驶逻辑（GoA3/4）
